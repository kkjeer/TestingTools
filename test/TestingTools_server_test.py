...
from TestingTools.TestingToolsImpl import TestingTools
from TestingTools.TestingToolsServer import MethodContext
...
class TestingToolsTest(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        for nameval in config.items('TestingTools'):
        ...
        cls.serviceImpl = TestingTools(cls.cfg)
    ...
    # NOTE: According to Python unittest naming rules test method names should start from 'test'. # noqa
    def test_run_TestingTools_ok(self):
        # call your implementation
        ret = self.serviceImpl.run_TestingTools(self.ctx,
    ...
    def test_run_TestingTools_min_len_negative(self):
        with self.assertRaisesRegex(ValueError, 'min_length parameter cannot be negative'):
            self.serviceImpl.run_TestingTools(self.ctx,
                                              {'workspace_name': self.wsName,
                                               'assembly_input_ref': '1/fake/3',
                                               'min_length': '-10'})

    def test_run_TestingTools_min_len_parse(self):
